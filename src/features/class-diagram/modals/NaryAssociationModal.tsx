...
export function NaryAssociationModal({ modal, project, expectedRevision, onRefreshProject }: { modal: Extract<Exclude<ModalState, null>, { type: 'addClassAssociation' }>; project: ProjectDto; expectedRevision: string; onRefreshProject: () => Promise<boolean> }) {
  ...
  const patchEnd = (id: string, patch: Partial<UmlAssociationEndDto>) => setEnds((current) => current.map((end) => end.id === id ? { ...end, ...patch } : end));
  const submit = async (event: FormEvent) => {
    ...
    try {
      await modelCommandApi.createAssociation(project.project.id, { expectedRevision, draft: { id, name: name.trim(), ends, associationClassId: null } });
...
function validate(name: string, ends: UmlAssociationEndDto[]) { const errors:string[]=[]; if(!name.trim()) errors.push('Association name is required.'); if(projectless(ends)) errors.push('Every end requires a classifier and role.'); for(const [index,end] of ends.entries()){const names=(end.qualifiers??[]).map(q=>q.name.trim());if(names.some(value=>!value)||new Set(names).size!==names.length)errors.push(`End ${index+1} contains invalid qualifier names.`);} return errors; }
function projectless(ends: UmlAssociationEndDto[]) { return ends.length < 2 || ends.some((end)=>!end.classId || !end.roleName.trim()); }
